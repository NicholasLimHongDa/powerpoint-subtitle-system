...
namespace ParsingTest1
{
    class Program
    {
        public static void OpenSesame(string result)
        {
            Process process = new Process();
            process.StartInfo.FileName = "cmd.exe";
            //process.StartInfo.Arguments = $@"/c echo {result} | cabocha";
        ...
        public static void Process(string output)
        {
            ...
            foreach (string unit in wordsinfo)
            {
                ...
                else if ((searchdump.Contains("副詞") == true) || (searchdump.Contains("形容動詞語幹") == true) || (searchdump.Contains("形容詞") == true) || (searchdump.Contains("連体化") == true) || (searchdump.Contains("連語") == true) || (searchdump.Contains("連体詞") == true))
                {
                    ...
                    else if ((displaylength >= 15) && (dependencyID[index] > (index + 1)))
                    {
                        goto ThresholdExceeded;
                    }
                ...
                {
                    //係り受け距離
                    if ((displaylength >= 15) && (dependencyID[index] > (index + 1)))
                    {
                        goto ThresholdExceeded;
                    }
            ...
            //Combines multiple 文節 and separates them by a space
            //Also states the total string length
            Console.WriteLine("===========================================");
            Console.WriteLine(string.Join(" ", bunsetsu));
            Console.WriteLine("String Length = " + stringlength);
            Console.WriteLine("===========================================");
            Console.WriteLine(String.Join("\t", dependencyscore));
            Console.WriteLine(String.Join("\t", phraseID));
            Console.WriteLine(String.Join("\t", dependencyID));
        ...
        {
            OpenSesame("プログラムとは何でしょう。なんとなくどんなものかわかっている方もいると思うんですけどプログラムっていうのはコンピュータを動かすための命令書みたいなものですね。ただコンピュータは日本語とか人の話す自然言語は今のところ理解できないですね。いずれ人の言葉を理解するようになると思うんですけどコンピュータが理解できるプログラミング言語っていうもので命令する必要があります。ただこのプログラミング言語いるいるあるんですけど我々が英語を覚えるとかいうよりは遥かに、簡単。単純な文法なのでとりあえずプログラミングの命令を１０個とかまあ30個命令と記号さえ覚えてしまえば、あとはプログラミング言語が持ってる素材をうまく組み合わせていくだけなんです。現説のプログラマでも全部と覚えているというわけではなくて調べながらググりながらプログラミングを組んでますね。昔と違ってあの素材って言いましたけれどもライブラリっていうのが揃っているのでいろいろなものプログラム初心者でも作れるようになりました");
            //OpenSesame("例えば環境の問題あるいは人口の問題新型コロナウイルスの蔓延など地球規模の問題たくさん生じておりますが残念ながらこれらの問題は将来にも継続しさらに悪くなってくると思われます"); //ここで入力
            //OpenSesame("人間の言語を機械で処理して内容を抽出することを自然言語処理と呼ぶ");
            //OpenSesame("国語の教え方については様々な方法がありますが当塾のメソッドはすべてオリジナルです");
            //OpenSesame("文間の関連の強さの二つ目の考え方は文間の数似度をそのまま関連の強さと考えてしまうというものである");
            //OpenSesame("この手法をもっとも単純に実現するには文の重要度を計算する際まず文中で他の文中の単語と意味的に関連がある単語の数を数えその単語数を文間の関連度として計算しその文間の関連度の総和を文の重要度とすればよいのだろう");
