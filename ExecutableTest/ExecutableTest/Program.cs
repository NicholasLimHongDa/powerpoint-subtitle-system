    ...
    class Program
    {
        ...
        public static void Process(string output)
        {
            ...
            foreach (string unit in wordsinfo)
            {
                ...
                //If the current line has more than 11 words, then it will not insert a linefeed (too short)
        ...
        async static Task Main(string[] args)
        {
            ...
            foreach (string unit in wordsinfo)
            {
                ...
                //If found at the last 文節, just inserts the 文節 as it is
                //「skip」is used to tell if the current 文節 has already been printed or not (because of bunsetsu[index + 1])
                else if ((searchdump.Contains("副詞") == true) || (searchdump.Contains("形容動詞語幹") == true) || (searchdump.Contains("形容詞") == true) || (searchdump.Contains("連体化") == true) || (searchdump.Contains("連語") == true))
                {
                    if (index >= (bunsetsu.Count - 1))
                    {
                        completedstring.Add(bunsetsu[index]);
                        //Do nothing might good in some cases (final bunsetsu duplicates may happen)
                        //OpenSesame("二つのテキスト要素が厳密に同じ意味を持つときこの二つの要素は意味的な等価である");
                    }
                    else
                    {
                        //If previous 文節 satisfies the same conditions, print only the 文節 after it (to prevent double printing)
                        if (skip == true)
                        {
                            completedstring.Add(bunsetsu[index + 1]);
                            index++;
                            continue;
